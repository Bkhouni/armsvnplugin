	...
	protected boolean executeSVNOperation(IDataProvider dataProvider)
		...
		if (file.isBinary()) {
			...
			try {
				...
				String content = ContentEncodingHelper.encodeBytes(
						file.getContent(), encoding);

				String fileExtension = FileUtil
						.getExtension(this.requestHandler.getUrl());
				if (fileExtension == null) {
					fileExtension = "txt";
				}
				this.fileData = new FileData(content, fileExtension);
