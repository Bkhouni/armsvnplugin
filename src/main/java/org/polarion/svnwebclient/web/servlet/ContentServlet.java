...
public class ContentServlet extends AbstractServlet {
	...
	protected void executeSVNOperation(IDataProvider dataProvider, State state)
		...
		if (type == IDataProvider.DIRECTORY) {
			this.forward(linkProvider.getDirectoryContentLink(), state);
		} else if (type == IDataProvider.FILE) {
			this.forward(linkProvider.getFileContentLink(), state);
		} else {
	...
	protected void forward(String page, State state)
			throws SVNWebClientException {
		try {
			SWCPluginHttpRequestWrapper.forward(state.getRequest(),
					state.getResponse(), page);
		} catch (Exception e) {
